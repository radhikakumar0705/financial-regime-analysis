...
REGIME_COLORS = {
    'Bull'                 : '#2ca02c',   # green
    'Bear'                 : '#d62728',   # red
    'High Volatility Bear' : '#ff7f0e',   # orange-red
    'Sideways'             : '#ff7f0e',   # orange / amber
    'High Volatility'      : '#9467bd',   # purple
}
...
#   Row 0 (tall)  : [Price + regime overlay — full width]
#   Row 1 (medium): [Regime bar strip — full width]
#   Row 2 (medium): [Return/Vol bars] [Transition heatmap] [Donut / Duration]
...
gs = gridspec.GridSpec(
    ...
    height_ratios=[2.0, 0.7, 1.6]
)


# PLOT 1 — Price series with regime background shading 

ax1 = fig.add_subplot(gs[0, :])

ax1.plot(df['date'], df['close'],
         color='#17202a', linewidth=0.8, alpha=0.92, zorder=3, label='NIFTY 50')
...
# PLOT 2 — Regime label strip per trading day 
...
ax2.legend(loc='upper right', ncol=N, fontsize=8, framealpha=0.9,
           markerscale=3)
ax2.set_xlim(ax1.get_xlim())
sns.despine(ax=ax2, left=True)


# PLOT 3 — Return & Volatility per regime 
...
bars1 = ax3.bar(x - w/2, ann_rets, w,
                color=[state_colors[s] for s in range(N)],
                alpha=0.85, edgecolor='white', linewidth=0.6,
                label='Ann. return %')
bars2 = ax3.bar(x + w/2, ann_vols, w,
                color=[state_colors[s] for s in range(N)],
                alpha=0.40, edgecolor='white', linewidth=0.6,
                hatch='//', label='Ann. vol %')
...
# PLOT 4 — Transition matrix heatmap  (row 2, col 1)
...
# PLOT 5 — Regime duration distribution  (row 2, col 2)
...
for s in range(N):
    d    = durations[s]
    mean = np.mean(d)
    ax5.hist(d, bins=bins,
             color=state_colors[s], alpha=0.55, edgecolor='white', linewidth=0.4,
             label=f'R{s} (μ={mean:.1f}d)')
    ax5.axvline(mean, color=state_colors[s], linewidth=1.6,
                linestyle='--', alpha=0.9)
